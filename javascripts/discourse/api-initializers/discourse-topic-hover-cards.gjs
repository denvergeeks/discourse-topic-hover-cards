...
const DELAY_SHOW  = settings.card_delay_ms ?? 300;  // ms before card appears
const DELAY_HIDE  = 200;                             // ms before card hides
const CARD_WIDTH  = settings.card_width ?? 420;
const VIEWPORT_MARGIN = 12;                          // px away from viewport edges
...
/** Format a compact number (1 234 → 1.2k) */
function fmtNum(n) {
  if (!n && n !== 0) return "0";
...
/** Build the static skeleton markup while the API is loading */
...
function buildCardHTML(topic) {
  ...
  // Category badge — prefer the existing Discourse badge markup if available
  let categoryHTML = "";
  if (topic.category_id) {
    const slug   = topic.category_slug ?? "";
  ...
  if (settings.show_views) {
    statItems.push(`<span class="topic-hover-card__stat">
      ${dIconSVG("eye")} ${fmtNum(topic.views)}
    </span>`);
  }
  if (settings.show_reply_count) {
    statItems.push(`<span class="topic-hover-card__stat">
      ${dIconSVG("comment")} ${fmtNum(topic.reply_count ?? topic.posts_count - 1)}
    </span>`);
  }
  if (settings.show_likes) {
    const likes = topic.like_count ?? topic.topic_post_like_count ?? 0;
    statItems.push(`<span class="topic-hover-card__stat">
      ${dIconSVG("heart")} ${fmtNum(likes)}
    </span>`);
  }

  if (statItems.length) {
    statsHTML = `<div class="topic-hover-card__stats">${statItems.join("")}</div>`;
  }
  ...
  if (settings.show_publish_date && topic.created_at) {
    ...
    publishDate = `<span class="topic-hover-card__publish-date">${formatted}</span>`;
  }
...
/**
 * Minimal inline SVG icons so the tooltip doesn't depend on Discourse's
 * icon sprite being loaded into the card markup (which lives outside .cooked).
 * Falls back to the same paths Discourse uses for FontAwesome 5.
 */
...
export default apiInitializer((api) => {
  // ---- State ----
  let tooltip       = null;    // the tooltip DOM element
  let showTimer     = null;
  let hideTimer     = null;
  let currentTopicId = null;   // topic currently shown/loading
  let topicCache    = {};      // { [topicId]: topicJSON }
  let isInsideCard  = false;   // mouse is over the tooltip itself

  // ---- Build / mount the tooltip DOM element once ----
    ...
    if (topicCache[topicId]) {
      ...
      fetchTopic(topicId).then((data) => {
        if (currentTopicId === topicId) {
          ...
          positionTooltip(anchorRect); // re-position after real height known
        }
      }).catch(() => {
        if (currentTopicId === topicId) {
  ...
  function hideCard() {
    ...
    // Delay resetting currentTopicId so re-enter of same link is instant
  ...
  //
  // We attach a single pair of listeners to document so that any link added
  // dynamically (cooked posts, infinite scroll, suggested topics, etc.) is
  // automatically handled without needing to re-register.
  //
  function onMouseEnter(event) {
    const link = event.target.closest("a[href]");
    if (!link) return;

    // Only trigger inside post/reply content and topic-list links
    ...
    scheduleShow(topicId, rect);
  }

  function onMouseLeave(event) {
    const link = event.target.closest("a[href]");
    if (!link) return;
    if (!topicIdFromHref(link.href)) return;
    scheduleHide();
  }

  // Use event delegation with capture so it fires on every link,
  // even those inside shadow DOM or re-rendered components.
  ...
  // Hide on scroll so the tooltip doesn't float away from its anchor
  document.addEventListener("scroll", () => {
    cancel(showTimer);
    hideCard();
  }, { passive: true, capture: true });

  // Clean up when navigating away (Ember router transition)
  api.onPageChange(() => {
    cancel(showTimer);
    cancel(hideTimer);
    hideCard();
